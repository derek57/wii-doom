...
#define SP_TIMEX		16
//#define SP_TIMEY		(SCREENHEIGHT-32)			// CHANGED FOR HIRES
...
/*
        patch_t tmp = { SCREENWIDTH, SCREENHEIGHT, 1, 1, 			// CHANGED FOR HIRES
...
void WI_drawEL(void)
{
    ...
    // draw "Entering"
/*
    V_DrawPatch((SCREENWIDTH - SHORT(entering->width))/2,			// CHANGED FOR HIRES
		y, 								// CHANGED FOR HIRES
...
/*
	if (left >= 0							// CHANGED FOR HIRES
	    && right < SCREENWIDTH					// CHANGED FOR HIRES
	    && top >= 0							// CHANGED FOR HIRES
	    && bottom < SCREENHEIGHT)					// CHANGED FOR HIRES
*/
	if (left >= 0							// CHANGED FOR HIRES
	    && right < ORIGWIDTH					// CHANGED FOR HIRES
	    && top >= 0							// CHANGED FOR HIRES
	    && bottom < ORIGHEIGHT)					// CHANGED FOR HIRES
	{
	    fits = true;
	}
	else
...
//    WI_drawPercent(SCREENWIDTH - SP_STATSX, SP_STATSY, cnt_kills[0]);		// CHANGED FOR HIRES
    WI_drawPercent(ORIGWIDTH - SP_STATSX, SP_STATSY, cnt_kills[0]);		// CHANGED FOR HIRES

    V_DrawPatch(SP_STATSX, SP_STATSY+lh, items);
//    WI_drawPercent(SCREENWIDTH - SP_STATSX, SP_STATSY+lh, cnt_items[0]);	// CHANGED FOR HIRES
    WI_drawPercent(ORIGWIDTH - SP_STATSX, SP_STATSY+lh, cnt_items[0]);		// CHANGED FOR HIRES

    V_DrawPatch(SP_STATSX, SP_STATSY+2*lh, sp_secret);
//    WI_drawPercent(SCREENWIDTH - SP_STATSX, SP_STATSY+2*lh, cnt_secret[0]);	// CHANGED FOR HIRES
    WI_drawPercent(ORIGWIDTH - SP_STATSX, SP_STATSY+2*lh, cnt_secret[0]);	// CHANGED FOR HIRES

    V_DrawPatch(SP_TIMEX, SP_TIMEY, timepatch);
//    WI_drawTime(SCREENWIDTH/2 - SP_TIMEX, SP_TIMEY, cnt_time);		// CHANGED FOR HIRES
    WI_drawTime(ORIGWIDTH/2 - SP_TIMEX, SP_TIMEY, cnt_time);			// CHANGED FOR HIRES

    if (wbs->epsd < 3)
    {
/*
	V_DrawPatch(SCREENWIDTH/2 + SP_TIMEX, SP_TIMEY, par);			// CHANGED FOR HIRES
	WI_drawTime(SCREENWIDTH - SP_TIMEX, SP_TIMEY, cnt_par);			// CHANGED FOR HIRES
*/
	V_DrawPatch(ORIGWIDTH/2 + SP_TIMEX, SP_TIMEY, par);			// CHANGED FOR HIRES
	WI_drawTime(ORIGWIDTH - SP_TIMEX, SP_TIMEY, cnt_par);			// CHANGED FOR HIRES
    }
