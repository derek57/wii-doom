...
char *P_TempSaveGameFile(void)
{
    ...
    sprintf(filename, "%s/temp.dsg", savegamedir);	// MODIFIED FOR THE WII (ADDED SEPARATOR)
...
char *P_SaveGameFile(int slot)
{
    ...
    sprintf(filename, "%s/%s", savegamedir, basename);	// MODIFIED FOR THE WII (ADDED SEPARATOR)
...
static byte saveg_read8(void)
{
    ...
    if (fread(&result, 1, 1, save_stream) < 1)
    {
        if (!savegame_error)
        {
            fprintf(statsfile, "saveg_read8: Unexpected end of file while "
                            "reading save game\n");
...
static void saveg_write8(byte value)
{
    if (fwrite(&value, 1, 1, save_stream) < 1)
    {
        if (!savegame_error)
        {
            fprintf(statsfile, "saveg_write8: Error while writing save game\n");
