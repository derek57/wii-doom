...
void R_RenderMaskedSegRange(drawseg_t *ds, int x1, int x2)
{
    ...
    colfunc = (curline->linedef->tranlump >= 0 && d_translucency ?
...
void R_StoreWallRange(int start, int stop)
{
    ...
    if (ds_p == drawsegs + maxdrawsegs)
    {
        ...
        if (maxdrawsegs_old)
            C_Printf(CR_GOLD, " R_StoreWallRange: Hit MAXDRAWSEGS limit at %d, raised to %d.\n",
    ...
    // killough 1/6/98, 2/1/98: remove limit on openings
    {
        ...
        if (need > maxopenings)
        {
            ...
            if(oldopenings != 0)
                C_Printf(CR_GOLD, " R_StoreWallRange: Hit MaxOpenings limit at %d, raised to %u\n",
