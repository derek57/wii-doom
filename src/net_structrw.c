...
dboolean NET_ReadConnectData(net_packet_t *packet, net_connect_data_t *data)
{
#ifdef DEBUG_NET
    printf("NET_ReadConnectData\n");
#endif
                         ...
                         dboolean lowres_turn)
{
    ...
    if (diff->diff & NET_TICDIFF_FORWARD)
        NET_WriteInt8(packet, diff->cmd.forwardmov);

    if (diff->diff & NET_TICDIFF_SIDE)
        NET_WriteInt8(packet, diff->cmd.sidemov);
...
#ifdef DEBUG_NET
    printf("NET_TiccmdDiff\n");
#endif

    if (tic1->forwardmov != tic2->forwardmov)
        diff->diff |= NET_TICDIFF_FORWARD;

    if (tic1->sidemov != tic2->sidemov)
...
#ifdef DEBUG_NET
    printf("NET_TiccmdPatch\n");
#endif

    // Apply the diff
    if (diff->diff & NET_TICDIFF_FORWARD)
        dest->forwardmov = diff->cmd.forwardmov;

    if (diff->diff & NET_TICDIFF_SIDE)
        dest->sidemov = diff->cmd.sidemov;
...
dboolean NET_ReadWaitData(net_packet_t *packet, net_waitdata_t *data)
{
    int i;

#ifdef DEBUG_NET
    printf("NET_ReadWaitData\n");
#endif
...
static dboolean NET_ReadBlob(net_packet_t *packet, uint8_t *buf, size_t len)
{
    unsigned int b;
    int i;
...
dboolean NET_ReadSHA1Sum(net_packet_t *packet, sha1_digest_t digest)
{
#ifdef DEBUG_NET
    printf("NET_ReadSHA1Sum\n");
#endif
...
/*
dboolean NET_ReadPRNGSeed(net_packet_t *packet, prng_seed_t seed)
{
