...
static int skill = 2;
static int no_monsters = 0;
static int dmatch = 0;
static int fast = 0;
static int respawn = 0;
static int udpport = 2342;
static int timer = 0;
static int privateserver = 1;
...
static dboolean IsChexQuest(const iwad_t *iwad)
{
...
static void LevelSelectDialog(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(user_data))
{
    ...
    if (warptype == WARP_ExMy)
    {
        int episodes = D_GetNumEpisodes(iwad->mission, iwad->mode);
...
static txt_dropdown_list_t *GameTypeDropdown(void)
{
    switch (gamemission)
    {
        case doom:
        default:
            return TXT_NewDropdownList(&dmatch, gamemodes, 3);
    }
}
...
static void StartGameMenu(char *window_title, int multiplayer)
{
    ...
    TXT_AddWidgets(window,
                   ...
                   TXT_NewInvertedCheckBox("Monsters enabled", &no_monsters),
