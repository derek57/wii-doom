...
//#define ST_MAPTITLEX (SCREENWIDTH - ST_MAPWIDTH * ST_CHATFONTWIDTH)	// CHANGED FOR HIRES
...
// main bar left
static patch_t*		sbar;			// HACK: ALL ELSE THAN SHAREWARE 1.0 & 1.1

static patch_t*		sbar_left_oldwad;	// HACK: IF SHAREWARE 1.0 OR 1.1
static patch_t*		sbar_right_oldwad;	// HACK: IF SHAREWARE 1.0 OR 1.1
...
{
    ST_loadData();
//    st_backing_screen = (byte *) Z_Malloc(ST_WIDTH * ST_HEIGHT, PU_STATIC, 0);			// CHANGED FOR HIRES
    st_backing_screen = (byte *) Z_Malloc((ST_WIDTH << hires) * (ST_HEIGHT << hires), PU_STATIC, 0);	// CHANGED FOR HIRES
}
